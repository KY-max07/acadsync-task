...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const isPublicPage = ['/', '/login', '/register', '/success'].includes(location.pathname);
  ...
  const hasPortalAccess = (portal: string) => {
      if (!subscription) return false;
      return subscription.portalsIncluded.includes(portal as any);
  };

  return (
    <div className="flex h-screen bg-white transition-colors duration-200 overflow-hidden">
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-black flex items-center justify-between px-4 z-40">
         <Link to="/" className="flex items-center gap-3">
             ...
             <span className="text-xl font-display italic">AcadSync</span>
         </Link>
      ...
      <aside 
        className={`
            fixed md:relative z-50 h-full w-64 bg-white border-r border-black flex flex-col transition-transform duration-300 ease-in-out
      ...
      >
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto" onClick={() => setIsSidebarOpen(false)}> {/* Close on link click */}
          <Link
            to="/dashboard"
            className={`flex items-center space-x-3 px-4 py-3 border border-transparent transition-all ${
              isActive('/dashboard') 
                ? 'bg-black text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]' 
                : 'text-gray-900 hover:border-black hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'
            }`}
          >
        ...
        <div className="p-4 border-t border-black space-y-2">
          ...
          <div className="bg-gray-100 p-3 rounded-lg mb-2">
             <p className="text-xs font-bold text-gray-500 uppercase mb-1">My Plan</p>
             <div className="flex items-center justify-between">
                <span className="font-bold text-black capitalize">
                    {subscription?.subscriptionType || 'Free'}
                </span>
                <Link to="/my-purchases" className="text-xs text-blue-600 hover:underline font-bold" onClick={() => setIsSidebarOpen(false)}>
                    Upgrade
                </Link>
             </div>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center space-x-3 px-4 py-2 text-black border border-black hover:bg-black hover:text-white transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
          >
            <LogOut size={20} />
            <span>Logout</span>
          </button>
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-auto p-4 md:p-8 relative bg-gray-50 pt-20 md:pt-8 w-full">
        {children}
      </main>
    </div>
  );
